...
namespace sklib
{
    ...
    {
    public:
        crc16(uint16_t initial_value = start_value_default)       { reset(initial_value); }
        ...
        void add(uint8_t ch)                      { accumulator = (uint16_t(accumulator >> 8) ^ transformation_table[(accumulator ^ ch) & 0xFF]); }
...
//        void add(uint16_t A)
    private:
        ...
        static constexpr uint16_t transformation_table[256] = {

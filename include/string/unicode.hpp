...
namespace unicode   // UNICODE designators (useful subset)
{
    ...
    static constexpr uint16_t HIGH_SURROGATE_CAP = HIGH_SURROGATE + HIGH_SURROGATE_LENGTH;
    static constexpr uint16_t LOW_SURROGATE_CAP  = LOW_SURROGATE + LOW_SURROGATE_LENGTH;
    ...
    static constexpr uint16_t REPLACEMENT_MARK = 0xFFFDu;   // also symbol for encoding error
...
{
    return is_clamped<make_unsigned_if_integer_type<T>>(to_unsigned_if_integer(ch), unicode::SURROGATE_START, unicode::SURROGATE_CAP-1);
}

template<class T>
constexpr bool is_unicode_high_surrogate(T ch)
{
    return is_clamped<make_unsigned_if_integer_type<T>>(to_unsigned_if_integer(ch), unicode::HIGH_SURROGATE, unicode::HIGH_SURROGATE_CAP-1);
}

template<class T>
constexpr bool is_unicode_low_surrogate(T ch)
{
    return is_clamped<make_unsigned_if_integer_type<T>>(to_unsigned_if_integer(ch), unicode::LOW_SURROGATE, unicode::LOW_SURROGATE_CAP-1);
}
...
{
    return is_clamped<make_unsigned_if_integer_type<T>>(to_unsigned_if_integer(ch), 0, unicode::PLANE_SIZE-1);
}

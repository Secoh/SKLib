...
namespace sklib
{
    namespace internal
    {
        using namespace ::sklib::internal::rs232_indexes;   // lets keep it local to the short block
...
#define SKLIB_INTERNAL_RS232_HELPER_FILL_ARRAY(arr,val) { \
 ...
 for (size_t i=0; i<SZ; i++) arr[i] = val;    }
        ...
        struct rs232_props
        {
            ...
            unsigned index_norm(const ::sklib::internal::rs232_indexes::props_group& group, unsigned mode, unsigned grp_count)
            {
                if (mode < 0) mode = 0;
            ...
            auto index_2_value(const ::sklib::internal::rs232_indexes::props_group& group, unsigned mode, const T (&arr)[N], const T* option = nullptr)
            {
...
#define PMEMBER(pack,field) ((pack) ? &((pack)->field) : nullptr)
                ...
                using namespace ::sklib::internal::rs232_indexes;   // keep local
...
{
    typedef ::sklib::internal::rs232_opaque_workspace_type workspace_type;
...
bool sklib::serial_io_type::open(unsigned com_port_no, unsigned mode, ::sklib::supplement::rs232_custom_options_type* option)
{
    ...
    ::sklib::internal::rs232_props Props{ mode, option };
